...
class AlexNet(nn.Module):
    def __init__(self, input_dim, hidden=512, num_classes=10):
        ...
        conv1 = []
        conv1.append(NormDistConv(3, 64, 3, 2, 1, bias=False))
        conv1.append(BoundMaxPool2d(3, 2, 0))
        self.conv1 = nn.ModuleList(conv1)

        conv2 = []
        conv2.append(NormDistConv(64, 192, 5, 1, 2, bias=False))
        conv2.append(BoundMaxPool2d(3, 2, 0))
        self.conv2 = nn.ModuleList(conv2)

        conv3 = []
        conv3.append(NormDistConv(192, 384, 3, 1, 1, bias=False))
        self.conv3 = nn.ModuleList(conv3)

        conv4 = []
        conv4.append(NormDistConv(384, 256, 3, 1, 1, bias=False))
