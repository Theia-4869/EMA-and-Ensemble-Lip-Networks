...
from model.model import Model, FusionModel, VotingModel, set_eps, get_eps
...
parser.add_argument('--model-num', default=10, type=int)
...
def main_worker(gpu, parallel, args, result_dir):
    ...
    print(ensemble_model.model_list)
    ...
    if output_flag:
        ...
        logger.print(model)
        logger.print('number of params: ', sum([p.numel() for p in model.parameters()]) * args.model_num)
    ...
    if args.optimizer == 'adamw':
        optimizer = AdamW(ensemble_model, lr=args.lr, weight_decay=args.wd, betas=(args.beta1, args.beta2), eps=args.epsilon)
    elif args.optimizer == 'madam':
        optimizer = Madam(ensemble_model, lr=args.lr)
    ...
    attacker = AttackPGD(ensemble_model, args.eps_test, step_size=args.eps_test / 4, num_steps=20, up=up, down=down)
    args.epochs = [int(epoch) for epoch in args.epochs.split(',')]
    schedule = create_schedule(args, len(train_loader), ensemble_model, loss, optimizer)
    ...
    for epoch in range(args.start_epoch, args.epochs[-1]):
        ...
        if epoch % 50 == 49:
            ...
            robust_train_acc = gen_adv_examples(ensemble_model, attacker, train_loader, gpu, parallel, logger, fast=True)
            robust_test_acc = gen_adv_examples(ensemble_model, attacker, test_loader, gpu, parallel, logger, fast=True)
            if writer is not None:
        ...
        if epoch % 5 == 4:
            certified_acc = certified_test(ensemble_model, args.eps_test, up, down, epoch, test_loader, logger, gpu, parallel)
            if writer is not None:
        ...
        if epoch > args.epochs[-1] - 3:
            ...
            gen_adv_examples(ensemble_model, attacker, test_loader, gpu, parallel, logger)
            certified_test(ensemble_model, args.eps_test, up, down, epoch, test_loader, logger, gpu, parallel)

    schedule(args.epochs[-1], 0)
    if output_flag:
        logger.print("Calculate certified accuracy on training dataset and test dataset")
    certified_test(ensemble_model, args.eps_test, up, down, args.epochs[-1], train_loader, logger, gpu, parallel)
    certified_test(ensemble_model, args.eps_test, up, down, args.epochs[-1], test_loader, logger, gpu, parallel)
